...
/*----------------------------------------------------------------------------*/
struct rto_hashstore*InitRTOHashstore()
{
  int i;
  struct rto_hashstore* hs = rte_calloc("rto_hashstore", 1, sizeof(struct rto_hashstore), 0);
  if (!hs) {
...
/*----------------------------------------------------------------------------*/
inline void AddtoRTOList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
...
/*----------------------------------------------------------------------------*/
inline void RemoveFromRTOList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
  if (cur_stream->on_rto_idx < 0) {
    //		assert(0);
    return;
...
/*----------------------------------------------------------------------------*/
inline void AddtoTimewaitList(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t cur_ts)
{
...
/*----------------------------------------------------------------------------*/
inline void RemoveFromTimewaitList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
  if (!cur_stream->on_timewait_list) {
    assert(0);
    return;
...
/*----------------------------------------------------------------------------*/
inline void AddtoTimeoutList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
...
/*----------------------------------------------------------------------------*/
inline void RemoveFromTimeoutList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
  if (cur_stream->on_timeout_list) {
...
/*----------------------------------------------------------------------------*/
inline void UpdateTimeoutList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
  if (cur_stream->on_timeout_list) {
    TAILQ_REMOVE(&mtcp->timeout_list, cur_stream, sndvar->timeout_link);
    TAILQ_INSERT_TAIL(&mtcp->timeout_list, cur_stream, sndvar->timeout_link);
...
/*----------------------------------------------------------------------------*/
inline void UpdateRetransmissionTimer(mtcp_manager_t mtcp, 
...
/*----------------------------------------------------------------------------*/
int HandleRTO(mtcp_manager_t mtcp, uint32_t cur_ts, tcp_stream *cur_stream)
{
  uint8_t backoff;
  ...
  if (cur_stream->sndvar->nrtx < TCP_MAX_RTX) {
	...
	printf("Connection lost, raise error event\n");
...
/*----------------------------------------------------------------------------*/
static inline void RearrangeRTOStore(mtcp_manager_t mtcp) {
...
/*----------------------------------------------------------------------------*/
void CheckRtmTimeout(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
  tcp_stream *walk, *next;
...
/*----------------------------------------------------------------------------*/
void CheckTimewaitExpire(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
  tcp_stream *walk, *next;
  int cnt;
...
/*----------------------------------------------------------------------------*/
void CheckConnectionTimeout(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
  tcp_stream *walk, *next;
  int cnt;
  ...
  for (walk = TAILQ_FIRST(&mtcp->timeout_list);
    ...
    if ((int32_t)(cur_ts - walk->last_active_ts) >= CONFIG.tcp_timeout) {
      ...
      if (walk->socket) {
	printf("Connection  timeout, raise error event\n");
