...
/*----------------------------------------------------------------------------*/
struct hashtable *CreateHashtable(unsigned int (*hashfn) (const tcp_stream *),         // key function
				  int (*eqfn) (const tcp_stream*, const tcp_stream *)) // equality
{
  int i;
  struct hashtable* ht = rte_calloc("hashtable", 1, sizeof(struct hashtable), 0);
  if (!ht){
...
/*----------------------------------------------------------------------------*/
int HTInsert(struct hashtable *ht, tcp_stream *item)
{
  /* create an entry*/ 
  int idx;
