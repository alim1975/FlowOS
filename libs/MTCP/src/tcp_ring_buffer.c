...
{
  rb_manager_t rbm = (rb_manager_t) rte_calloc("rb_manager", 1, sizeof(rb_manager), 0);
  ...
  //printf("Create FreeFragQueue of size %d\n", cnum);
...
AllocateFragmentContext(rb_manager_t rbm)
{
  ...
  if (!frag) {
	...
	frag = rte_calloc("fragment_ctx", 1, sizeof(struct fragment_ctx), 0);
	if (frag == NULL) {
...
struct tcp_ring_buffer *RBInit(rb_manager_t rbm, uint32_t init_seq)
{
  struct tcp_ring_buffer* buff = 
    (struct tcp_ring_buffer*)rte_calloc("tcp_ring_buffer", 1, sizeof(struct tcp_ring_buffer), 0);
	  ...
	  void* data, uint32_t len, uint32_t cur_seq)
{
  ...
  if (!merged) {
    if (buff->fctx == NULL) {
      ...
      /* if the seqnum is in-between the fragments or at the last */
