...
#define RX_POOL_SIZE RTE_MAX(NB_PORTS * NB_RX_QUEUE *  NB_RX_DESC + CACHE_SIZE + NB_PORTS * MAX_PKT_BURST, 8192)

#define RX_POOL_SIZE RTE_MAX(NB_PORTS * NB_TX_QUEUE *  NB_TX_DESC + CACHE_SIZE + NB_PORTS * MAX_PKT_BURST, 8192)
...
static void RunMainLoop(struct mtcp_thread_context *ctx)
{
  ...
  while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {    
    ...
    rte_pktmbuf *packet = flowos_dequeue(mtcp->rx_queue);
    //  for (i = 0; i < recv_cnt; i++) {
    //printf("Process packet %d\n", i);
    if ( packet ) {
      ProcessPacket(mtcp, idx, ts, packet);
...
/*---------------------------------------------------------------------------
 * TODO: TCP thread process data from it's RX queue
 */
static void *MTCPRunThread(void *arg)
{
  ...
  SQ_LOCK_INIT(&ctx->rx_lock, "ctx->rx_lock", exit(-1));
  SQ_LOCK_INIT(&ctx->tx_lock, "ctx->tx_lock", exit(-1));
