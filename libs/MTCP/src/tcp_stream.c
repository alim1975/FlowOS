...
/*---------------------------------------------------------------------------*/
inline char *TCPStateToString(const tcp_stream *stream)
{
  return state_str[stream->state];
}
/*---------------------------------------------------------------------------*/
unsigned int HashFlow(const tcp_stream *flow)
{
#if 0
...
/*---------------------------------------------------------------------------*/
int EqualFlow(const tcp_stream *flow1, const tcp_stream *flow2)
{
...
inline void RaiseReadEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
  if (stream->socket) {
    if (stream->socket->epoll & MTCP_EPOLLIN) {
      AddEpollEvent(mtcp->ep, MTCP_EVENT_QUEUE, stream->socket, MTCP_EPOLLIN);
#if BLOCKING_SUPPORT
    } 
...
inline void RaiseWriteEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
  if (stream->socket) {
    if (stream->socket->epoll & MTCP_EPOLLOUT) {
      AddEpollEvent(mtcp->ep, MTCP_EVENT_QUEUE, stream->socket, MTCP_EPOLLOUT);
#if BLOCKING_SUPPORT
    } 
    else if (!(stream->socket->opts & MTCP_NONBLOCK)) {
...
inline void RaiseErrorEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
  if (stream->socket) {
    if (stream->socket->epoll & MTCP_EPOLLERR) {
      AddEpollEvent(mtcp->ep, MTCP_EVENT_QUEUE, stream->socket, MTCP_EPOLLERR);
#if BLOCKING_SUPPORT
    } 
    else if (!(stream->socket->opts & MTCP_NONBLOCK)) {
      if (!stream->on_rcv_br_list) {
...
/*---------------------------------------------------------------------------*/
tcp_stream *CreateTCPStream(mtcp_manager_t mtcp, socket_map_t socket, int type, 
			    uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport)
{
  ...
  if (ret < 0) {
    TRACE_ERROR("Stream %d: Failed to insert the stream into hash table.\n", stream->id);
    MPFreeChunk(mtcp->flow_pool, stream);
    pthread_mutex_unlock(&mtcp->ctx->flow_pool_lock);
    return NULL;
  }
...
void DumpStream(mtcp_manager_t mtcp, tcp_stream *stream)
{
  ...
  thread_printf(mtcp, mtcp->log_fp, "========== Receive variables ==========\n");
  thread_printf(mtcp, mtcp->log_fp, 
