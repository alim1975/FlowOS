...
/*----------------------------------------------------------------------------*/
sb_manager_t SBManagerCreate(size_t chunk_size, uint32_t cnum)
{
  sb_manager_t sbm = (sb_manager_t)rte_calloc("sb_manager", 1, sizeof(sb_manager), 0);
  if (!sbm) {
...
struct tcp_send_buffer *SBInit(sb_manager_t sbm, uint32_t init_seq)
{
  ...
  if (!buf) {
    buf = (struct tcp_send_buffer *)rte_malloc("tcp_send_buffer", sizeof(struct tcp_send_buffer), 0);
    if (!buf) {
...
/*----------------------------------------------------------------------------*/
#if 0
static void SBFreeInternal(sb_manager_t sbm, struct tcp_send_buffer *buf)
{
  if (!buf)
    return;
...
/*----------------------------------------------------------------------------*/
size_t SBPut(sb_manager_t sbm, struct tcp_send_buffer *buf, void *data, size_t len)
{
  size_t to_put;
