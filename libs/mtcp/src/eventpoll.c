...
struct event_queue *CreateEventQueue(int size)
{
  ...
  eq = (struct event_queue *) rte_calloc("event_queue", 1, sizeof(struct event_queue), 0);
  if (!eq) return NULL;
  ...
  eq->events = (struct mtcp_epoll_event_int *)
    rte_calloc("mtcp_epoll_event_int", size, sizeof(struct mtcp_epoll_event_int), 0);
  if (!eq->events) {
    rte_free(eq);
    return NULL;
  }
...
{
  if (eq->events) rte_free(eq->events);
  rte_free(eq);
}
/*----------------------------------------------------------------------------*/
int mtcp_epoll_create(mctx_t mctx, int size)
{
  ...
  ep = (struct mtcp_epoll *) rte_calloc("mtcp_epoll", 1, sizeof(struct mtcp_epoll), 0);
  if (!ep) {
...
/*----------------------------------------------------------------------------*/
static int RaisePendingStreamEvents(mtcp_manager_t mtcp, 
...
/*----------------------------------------------------------------------------*/
int mtcp_epoll_ctl(mctx_t mctx, int epid, 
