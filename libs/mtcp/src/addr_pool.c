...
addr_pool_t CreateAddressPool(in_addr_t addr_base, int num_addr)
{
  ...
  ap = (addr_pool_t)rte_calloc("addr_pool", 1, sizeof(struct addr_pool), 0);
  if (!ap) return NULL;

  /* initialize address pool */
  num_entry = num_addr * (MAX_PORT - MIN_PORT);
  ap->pool = (struct addr_entry *) rte_calloc("addr_entry", num_entry, sizeof(struct addr_entry), 0);
  if (!ap->pool) {
    rte_free(ap);
    return NULL;
  }
  ...
  ap->mapper = (struct addr_map *) rte_calloc("addr_map", num_addr, sizeof(struct addr_map), 0);
  if (!ap->mapper) {
    rte_free(ap->pool);
    rte_free(ap);
    return NULL;
