...
/*----------------------------------------------------------------------------*/
static inline int mtcp_is_connected(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
  if (!cur_stream) {
...
/*----------------------------------------------------------------------------*/
inline mtcp_manager_t GetMTCPManager(mctx_t mctx)
{
  if (!mctx) {
    errno = EINVAL;
    return NULL;
...
/*----------------------------------------------------------------------------*/
inline int GetSocketError(socket_map_t socket, void *optval, socklen_t *optlen)
{
...
/*----------------------------------------------------------------------------*/
int mtcp_getsockopt(mctx_t mctx, int sockid, int level, 
...
/*----------------------------------------------------------------------------*/
int mtcp_setsockopt(mctx_t mctx, int sockid, int level, 
...
/*----------------------------------------------------------------------------*/
int mtcp_bind(mctx_t mctx, int sockid, 
...
int mtcp_listen(mctx_t mctx, int sockid, int backlog)
{
  ...
  listener = (struct tcp_listener *) rte_calloc("tcp_listener", 1, sizeof(struct tcp_listener), 0);
...
/*----------------------------------------------------------------------------*/
int mtcp_init_rss(mctx_t mctx, in_addr_t saddr_base, int num_addr, 
...
int mtcp_connect(mctx_t mctx, int sockid, const struct sockaddr *addr, socklen_t addrlen)
{
  ...
  mtcp = GetMTCPManager(mctx);
  if (!mtcp) {
    printf("mtcp_connect(): failed to retrieve MTCP context...\n");
    return -1;
  }
  if (sockid < 0 || sockid >= CONFIG.max_concurrency) {
    printf("mtcp_connect(): socket id %d out of range.\n", sockid);
    errno = EBADF;
    return -1;
  }

  if (mtcp->smap[sockid].socktype == MTCP_SOCK_UNUSED) {
    printf("mtcp_connect(): invalid socket id: %d\n", sockid);
    errno = EBADF;
    return -1;
  }
  
  if (mtcp->smap[sockid].socktype != MTCP_SOCK_STREAM) {
    printf("mtcp_connect(): not an end socket id: %d\n", sockid);
    errno = ENOTSOCK;
    return -1;
  }
  
  if (!addr) {
    printf("mtcp_connect(): socket %d: empty address!\n", sockid);
    errno = EFAULT;
    return -1;
  }
  ...
  /* we only allow bind() for AF_INET address */
  if (addr->sa_family != AF_INET || addrlen < sizeof(struct sockaddr_in)) {
    printf("mtcp_connect(): socket %d: invalid argument!\n", sockid);
    errno = EAFNOSUPPORT;
    return -1;
  }
  ...
  socket = &mtcp->smap[sockid];
  if (socket->stream) {
    printf("mtcp_connect(): socket %d: stream already exist!\n", sockid);
  ...
  if (socket->opts & MTCP_ADDR_BIND) {
    ...
    if (rss_core != mctx->cpu) {
      printf("mtcp_connect(): socket: %d rss core != running core\n", sockid);
      errno = EINVAL;
      return -1;
    }
  } 
    ...
    if (mtcp->ap) {
      ret = FetchAddress(mtcp->ap, mctx->cpu, num_queues, addr_in, &socket->saddr);
    }
    else {
      ret = FetchAddress(ap, mctx->cpu, num_queues, addr_in, &socket->saddr);
    }
    if (ret < 0) {
      printf("mtcp_connect(): socket %d: failed to retrieve IP address\n", sockid);
      errno = EAGAIN;
      return -1;
    }
  ...
  if (!cur_stream) {
    printf("mtcp_connect(): socket %d: failed to create tcp_stream!\n", sockid);
    errno = ENOMEM;
    return -1;
  }
  ...
  if (ret < 0) {
    printf("mtcp_connect(): socket %d: failed to enqueue to conenct queue!\n", sockid);
  ...
  if (socket->opts & MTCP_NONBLOCK) {
    errno = EINPROGRESS;
    printf("mtcp_connect(): socket %d: socket is non-blocking\n", sockid);
    return -1;
    ...
    while (1) {
      if (!cur_stream) {
	printf("mtcp_connect(): STREAM DESTROYED\n");
	errno = ETIMEDOUT;
	return -1;
      }
      if (cur_stream->state > TCP_ST_ESTABLISHED) {
	printf("mtcp_connect(): socket %d: weird state %s\n", 
...
/*----------------------------------------------------------------------------*/
static inline int CloseStreamSocket(mctx_t mctx, int sockid)
{
  mtcp_manager_t mtcp;
  tcp_stream *cur_stream;
  int ret;
...
/*----------------------------------------------------------------------------*/
static inline int CloseListeningSocket(mctx_t mctx, int sockid)
{
  mtcp_manager_t mtcp;
  struct tcp_listener *listener;
...
/*----------------------------------------------------------------------------*/
static inline int CopyToUser(mtcp_manager_t mtcp, tcp_stream *cur_stream, char *buf, int len)
{
...
int mtcp_read(mctx_t mctx, int sockid, char *buf, int len)
{
  ...
  mtcp = GetMTCPManager(mctx);
  if (!mtcp) {
    printf("mtcp_read(): socket %d: failed to retrieve TCP conext\n", sockid);
    return -1;
  }
  if (sockid < 0 || sockid >= CONFIG.max_concurrency) {
    TRACE_API("Socket id %d out of range.\n", sockid);
...
/*----------------------------------------------------------------------------*/
static inline int CopyFromUser(mtcp_manager_t mtcp, tcp_stream *cur_stream, char *buf, int len)
{
