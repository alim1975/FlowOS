...
int mtcp_pipe(mctx_t mctx, int pipeid[2])
{
  ...
  pp = (struct pipe *) rte_calloc("pipe", 1, sizeof(struct pipe), 0);
  if (!pp) {
    /* errno set by calloc() */
    FreeSocket(mctx, socket[0]->id, FALSE);
  ...
  pp->buf = (char *) rte_malloc("char*", pp->buf_size, 0);
  if (!pp->buf) {
    /* errno set by malloc() */
    FreeSocket(mctx, socket[0]->id, FALSE);
    FreeSocket(mctx, socket[1]->id, FALSE);
...
/*---------------------------------------------------------------------------*/
static void RaiseEventToPair(mtcp_manager_t mtcp, socket_map_t socket, uint32_t event)
{
...
/*---------------------------------------------------------------------------*/
int PipeRead(mctx_t mctx, int pipeid, char *buf, int len)
{
  mtcp_manager_t mtcp;
  socket_map_t socket;
  struct pipe *pp;
...
/*---------------------------------------------------------------------------*/
int PipeWrite(mctx_t mctx, int pipeid, char *buf, int len)
{
  mtcp_manager_t mtcp;
  socket_map_t socket;
  struct pipe *pp;
...
/*----------------------------------------------------------------------------*/
int RaisePendingPipeEvents(mctx_t mctx, int epid, int pipeid)
{
  ...
  if (!pp) return -1;
  if (pp->state < PIPE_ACTIVE) return -1;
...
/*---------------------------------------------------------------------------*/
int PipeClose(mctx_t mctx, int pipeid)
{
  mtcp_manager_t mtcp;
  socket_map_t socket;
  struct pipe *pp;
