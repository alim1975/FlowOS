			    ...
			    uint32_t cur_ts, uint32_t echo_ts)
{
  ...
  tcph = (struct tcphdr *)IPOutputStandalone(mtcp, 0, saddr, daddr, TCP_HEADER_LEN + optlen + payloadlen);
  if (tcph == NULL) return ERROR;
  ...
  if (flags & TCP_FLAG_SYN) tcph->syn = TRUE;
  if (flags & TCP_FLAG_FIN) tcph->fin = TRUE;
  if (flags & TCP_FLAG_RST) tcph->rst = TRUE;
  if (flags & TCP_FLAG_PSH) tcph->psh = TRUE;
  ...
  tcph->check = TCPCalcChecksum((uint16_t *)tcph, TCP_HEADER_LEN + optlen + payloadlen, saddr, daddr);
#endif
  
  if (tcph->syn || tcph->fin) payloadlen++;
		  ...
		  uint32_t cur_ts, uint8_t flags, uint8_t *payload, uint16_t payloadlen)
{
  ...
  tcph = (struct tcphdr *)IPOutput(mtcp, cur_stream, TCP_HEADER_LEN + optlen + payloadlen);
  if (tcph == NULL) {
    TRACE_ERROR("SendTCPPacket(): failed to get TCP segment to send\n");
    return -2;
  }  
  memset(tcph, 0, TCP_HEADER_LEN + optlen);
  ...
  if (flags & TCP_FLAG_SYN) wscale = 0; 
  else wscale = cur_stream->sndvar->wscale;
  ...
  if (window32 == 0) cur_stream->need_wnd_adv = TRUE;
...
/*----------------------------------------------------------------------------*/
static int FlushTCPSendingBuffer(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t cur_ts)
{
  ...
  if (sndvar->sndbuf->len == 0) return 0;
  ...
  while (1) {
    ...
    if (buffered_len > maxlen) len = maxlen;
    else len = buffered_len;
...
/*----------------------------------------------------------------------------*/
static inline int SendControlPacket(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t cur_ts)
{
  struct tcp_send_vars *sndvar = cur_stream->sndvar;
  int ret = 0;
...
/*----------------------------------------------------------------------------*/
inline int WriteTCPControlList(mtcp_manager_t mtcp, struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
  ...
  //printf("Send TCP control messages\n");
  ...
  while (cur_stream) {
    ...
    if (cur_stream->sndvar->on_control_list) {
      ...
      //printf("Stream %u: Sending control packet\n", cur_stream->id);
...
/*----------------------------------------------------------------------------*/
inline int WriteTCPDataList(mtcp_manager_t mtcp, struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
  tcp_stream *cur_stream;
  tcp_stream *next, *last;
  int cnt = 0;
...
/*----------------------------------------------------------------------------*/
inline int WriteTCPACKList(mtcp_manager_t mtcp, struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
  tcp_stream *cur_stream;
  tcp_stream *next, *last;
  int to_ack;
...
/*----------------------------------------------------------------------------*/
inline struct mtcp_sender *GetSender(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
...
inline void AddtoControlList(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t cur_ts)
{
  ...
  assert(sender != NULL);
  //printf("Send control packet before queueing\n");
  ret = SendControlPacket(mtcp, cur_stream, cur_ts);
  if (ret < 0) {
#endif
    if (!cur_stream->sndvar->on_control_list) {
      ...
      sender->control_list_cnt++;
      //printf("Stream %u: added to control list (cnt: %d)\n", 
    ...
    if (cur_stream->sndvar->on_control_list) {
      //printf("Stream %u: removed from control list (cnt: %d)\n", 
      //     cur_stream->id, sender->control_list_cnt);
      cur_stream->sndvar->on_control_list = FALSE;
      TAILQ_REMOVE(&sender->control_list, cur_stream, sndvar->control_link);
      sender->control_list_cnt--;
...
/*----------------------------------------------------------------------------*/
inline void AddtoACKList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
  struct mtcp_sender *sender = GetSender(mtcp, cur_stream);
  assert(sender != NULL);
