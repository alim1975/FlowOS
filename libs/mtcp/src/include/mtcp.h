...
struct mtcp_manager
{
  ...
  stream_queue_t rx_queue;				/* streams need to to process */
  
    stream_queue_t tx_queue;				/* streams need to send */
...
struct mtcp_thread_context
{
  int cpu;
  pthread_t thread;
  uint8_t done:1, exit:1, interrupt:1;
  
  struct rte_mempool *tx_pool;
  struct rte_mempool *rx_pool;
  struct mtcp_manager* mtcp_manager;
  
  struct dpdk_burst w_chunk[ETH_NUM];
