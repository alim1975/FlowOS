...
unsigned char *flowos_get_mac_address(uint32_t ip) {
  ...
  for (i = 0; i < flowos.device_count; i++) {
    if (ip == flowos.devices[i].ip_addr) {
      haddr = flowos.devices[i].mac_addr;
      break;
    }
  }
  return haddr;
...
unsigned char *flowos_get_dest_mac_address(uint32_t dip) {
  ...
  for (i = 0; i < flowos.arp.entries; i++) {
    if (flowos.arp.entry[i].prefix == 1) {
      if (flowos.arp.entry[i].ip == dip) {
				d_haddr = flowos.arp.entry[i].haddr;
				break;
      }	
    } 
    else {
      if ((dip & flowos.arp.entry[i].ip_mask) == flowos.arp.entry[i].ip_masked) {				
				if (flowos.arp.entry[i].prefix > prefix) {
					d_haddr = flowos.arp.entry[i].haddr;
					prefix = flowos.arp.entry[i].prefix;
				}
      }
    }
  }
...
/*--------------------------------------------------------------------*/
static int flowos_arp_output(int nif, int opcode,
														 uint32_t dst_ip, 
  ...
  struct arphdr *arph = (struct arphdr *)
    flowos_eth_output(ETH_P_ARP, nif, dst_haddr, sizeof(struct arphdr));
  if (! arph) {
  ...
  /* Fill arp body */
  arph->ar_sip = flowos.devices[nif].ip_addr;
  arph->ar_tip = dst_ip;
  
  memcpy(arph->ar_sha, flowos.devices[nif].mac_addr, arph->ar_hln);
...
int flowos_register_arp_entry(uint32_t ip, const unsigned char *haddr) {
  int idx = flowos.arp.entries;
  
  flowos.arp.entry[idx].prefix = 32;
  flowos.arp.entry[idx].ip = ip;
  memcpy(flowos.arp.entry[idx].haddr, haddr, ETH_ALEN);
  flowos.arp.entry[idx].ip_mask = -1;
  flowos.arp.entry[idx].ip_masked = ip;
  
  flowos.arp.entries = idx + 1;
...
/*--------------------------------------------------------------------*/
void flowos_send_arp_request(uint32_t ip, int nif, uint32_t cur_ts) {
  ...
  flowos_arp_output(nif, arp_op_request, ip, haddr);
}
/*-------------------------------------------------------------------*/
static int flowos_process_arp_request(struct rte_mbuf *pkt,
																			int nif, 
																			uint32_t cur_ts) {
  unsigned char *temp;
  struct arphdr *arph = (struct arphdr *)
  ...
  /* send ARP reply */
  flowos_arp_output(nif, arp_op_reply, arph->ar_sip, arph->ar_sha);
 
  rte_pktmbuf_free(pkt);
  return 0;
}
/*----------------------------------------------------------------------------*/
static int flowos_process_arp_reply(struct rte_mbuf *pkt,
																		uint32_t cur_ts) {
  unsigned char *temp;
  struct arp_queue_entry *ent;
  struct arphdr *arph = (struct arphdr *)
...
/*-------------------------------------------------------------------*/
int flowos_process_arp_packet(uint32_t cur_ts,
															const int idx, 
															struct rte_mbuf *pkt) {
  struct arphdr *arph = (struct arphdr *)
  ...
  for (i = 0; i < flowos.device_count; i++) {
    if (arph->ar_tip == flowos.devices[i].ip_addr) {
      to_me = TRUE;
    }
  }
  
  if (! to_me) {
    //printf("ARP packet is not for me, dropping\n");
		//flowos_dump_arp_packet(arph);
    rte_pktmbuf_free(pkt);
    return TRUE;
  }
  ...
  case arp_op_request:
    flowos_process_arp_request(pkt, idx, cur_ts);
    break;
    
  case arp_op_reply:
    flowos_process_arp_reply(pkt, cur_ts);
    break;
...
/*-------------------------------------------------------------------*/
// Publish my address
void flowos_advertise_mac_address() {
  int i;
  for (i = 0; i < flowos.device_count; i++) {
    flowos_arp_output(i, arp_op_request, 0, NULL);
  }
}
/*-------------------------------------------------------------------*/
void flowos_print_arp_table() {
  ...
  for (i = 0; i < flowos.arp.entries; i++) {
    uint8_t *da = (uint8_t *)&flowos.arp.entry[i].ip;
    
    printf("IP addr: %u.%u.%u.%u, "
		 "dst_mac: %02X:%02X:%02X:%02X:%02X:%02X\n",
		 da[0], da[1], da[2], da[3],
		 flowos.arp.entry[i].haddr[0],
		 flowos.arp.entry[i].haddr[1],
		 flowos.arp.entry[i].haddr[2],
		 flowos.arp.entry[i].haddr[3],
		 flowos.arp.entry[i].haddr[4],
		 flowos.arp.entry[i].haddr[5]);
  }
  if (flowos.arp.entries == 0)
    printf("(blank)\n");
...
void flowos_dump_arp_packet(struct arphdr *arph) {
  ...
  fprintf(stderr, "FlowOS: ARP header \n");
